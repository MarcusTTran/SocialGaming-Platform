...
void RulesRunner::configurationSetter(const Configuration &config) { this->configuration = config; }
void RulesRunner::variablesSetter(const DataValue::OrderedMapType &vars) { variables = vars; }
void RulesRunner::constantsSetter(const DataValue::OrderedMapType &consts) { constants = consts; }
void RulesRunner::perPlayerSetter(const DataValue::OrderedMapType &perPlayers) { perPlayer = perPlayers; }
void RulesRunner::perAudienceSetter(const DataValue::OrderedMapType &perAudiences) { perAudience = perAudiences; }
void RulesRunner::rulesSetter(const std::vector<Rule> &r) { rules = r; }
DataValue::OrderedMapType RulesRunner::constantsGetter() { return constants; }
...
void RulesRunner::processRule(const std::vector<Rule> &rules,
    ...
    std::for_each(rules.begin(), rules.end(), [&](const Rule &rule) { processRule(rule, processRule); });
}
...
void RulesRunner::printMessageWithPlaceholders(const Rule &rule, const std::string &key,
...
void RulesRunner::implementForRule(const Rule &rule, std::unordered_map<std::string, std::string> &loopVariables) {
    ...
    auto it = std::find_if(rule.parameters.begin() + 1, rule.parameters.end(), [&](const auto &param) {
        if (auto str = std::get_if<std::string>(&param);
            str && std::find(GameConstantsType::builtin.begin(), GameConstantsType::builtin.end(), *str) !=
                       GameConstantsType::builtin.end()) {
            builtinFunction = *str;
            return true;
        }
        if (auto str = std::get_if<std::string>(&param)) {
    ...
    if (rootKey == "configuration")
        currentContainer = std::make_shared<decltype(configuration.setup)>(configuration.setup);
    else if (rootKey == "constants")
        currentContainer = std::make_shared<decltype(constants)>(constants);
    else if (rootKey == "variables")
        currentContainer = std::make_shared<decltype(variables)>(variables);
    else if (rootKey == "perPlayer")
        currentContainer = std::make_shared<decltype(perPlayer)>(perPlayer);
    else if (rootKey == "perAudience")
        currentContainer = std::make_shared<decltype(perAudience)>(perAudience);
    else {
    ...
    if (setupContainer && targetPath.size() > 1) {
        ...
        for (const auto &setupEntry : *setupContainer) {
            auto it = std::find_if(
                setupEntry.begin(), setupEntry.end(),
                [&target](const std::pair<std::string, DataValue> &entry) { return entry.first == target; });
    ...
    if (builtinFunction && *builtinFunction == "upfrom") {
        ...
        int end = -1;
        auto rangeIt =
            std::find_if(orderedMap.begin(), orderedMap.end(),
                         [](const std::pair<std::string, DataValue> &entry) { return entry.first == "range"; });

        if (rangeIt != orderedMap.end() && rangeIt->second.getType() == "RANGE") {
            auto rangeValue = rangeIt->second.asRange();
            end = rangeValue.second;
        } else {
            std::cerr << "'range' not found or is not a valid range type in " << targetPath.back() << " entry."
                      << std::endl;
        }
...
void RulesRunner::processRules() {
    std::for_each(begin(rules), end(rules), [this](const Rule &rule) { this->printRule(rule, 0); });
